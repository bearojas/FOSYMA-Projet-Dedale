	...
	public HashMap<String,Data<List<String>,String, List<Attribute>, List<AID>>> graphToHashmap(Graph graphToSend, AID receiver){
		...
		for (Node n : graphToSend){
			...
			if( lVisitor.indexOf(receiver) == -1 || ((List<Attribute>) lattribute).indexOf("TREASURE")!=-1){
				...
				//TODO
				/*
				 * On a envoy� une liste contenant le nom de l'agent r�cepteur lVisitor
				 * l'agent qui re�oit n'a donc rien a chang�
				 * l'agent qui envoit ne notifie pas qu'il a deja envoy�, il modifiera quand l'autre agent lui renverra la liste avec son nom
				 */
